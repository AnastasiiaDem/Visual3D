...
export class Lab1Component implements OnInit, AfterViewInit {
  ...
  private startRenderingLoop() {
    ...
    let component: Lab1Component = this;
    (function render() {
  ...
  rotate() {
    for (let i = 0; i < 3; i++) {
      this.lineWire[i].rotation.x = (Math.PI * this.xAngle) / 180;
      this.lineWire[i].rotation.y = (Math.PI * this.yAngle) / 180;
      this.lineWire[i].rotation.z = (Math.PI * this.zAngle) / 180;
  ...
  move() {
    for (let i = 0; i < 3; i++) {
      this.lineWire[i].position.x = this.xPos;
      this.lineWire[i].position.y = this.yPos;
      this.lineWire[i].position.z = this.zPos;
      this.scene.add(this.lineWire[i]);
    }
    this.computeBezierSurface();
  }
  ...
  scale() {
    for (let i = 0; i < 3; i++) {
      this.lineWire[i].scale.x = this.xScale;
      this.lineWire[i].scale.y = this.yScale;
      this.lineWire[i].scale.z = this.zScale;
      this.scene.add(this.lineWire[i]);
    }
    this.computeBezierSurface();
  }
  ...
  renderSurface(i: number) {
    this.scene.remove(this.surfaceWire[i]);
    this.scene.remove(this.lineWire[i]);
